...
\item \textbf{Top-down sampling algorithm:} To sample the leaves that with odd sum, we use Bayesian theorem so that (for simplicity, $T$ and $\Theta$ are omitted)
\begin{align*}
	p(X_{r(T)} \vert \mathfrak{O}\{T\}) = \frac{p(\mathfrak{O}\{T\} \vert X_{r(T)}) p(X_{r(T)})}{p(\mathfrak{O}\{T\})}.
\end{align*}
\par Then, if we look at the left child of 
\end{enumerate}
